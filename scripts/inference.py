...
class Diff2FlowEulerSampler:
    ...
    def _convert_fm_to_dm(self, fm_t: torch.Tensor, fm_x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Maps Flow Matching (t, x) to Diffusion Model (t, x)."""
        # FIX 1: Clamp the TIME VALUE itself, not the index. 
        # This prevents dm_t from extrapolating to 1015.6 and breaking UNet embeddings!
        fm_t = torch.clamp(
            fm_t, 
            min=float(self.rectified_alphas_flipped[0]), 
            max=float(self.rectified_alphas_flipped[-1])
        )
    ...
    def _predict_velocity(self, dm_t: torch.Tensor, dm_x: torch.Tensor, eps_pred: torch.Tensor) -> torch.Tensor:
...
class QwenSDXLPipeline:
    ...
    def __init__(self, adapter_ckpt_path: str, device: str = "cuda"):
        ...
        self.qwen = AutoModelForCausalLM.from_pretrained(config.qwen_model_id, torch_dtype=self.dtype).to(self.device).eval()
        
        # FIX 2: VAE MUST BE LOADED IN FLOAT16! The Ollin fp16-fix explodes in bfloat16.
        self.vae = AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix", torch_dtype=torch.float16).to(self.device).eval()
        
        self.unet = UNet2DConditionModel.from_pretrained(config.sdxl_model_id, subfolder="unet", torch_dtype=self.dtype).to(self.device).eval()
        ...
        self.adapter = CausalToSpatialPerceiverBridge(
            depth=config.adapter_depth, qwen_dim=config.adapter_dim, internal_dim=config.adapter_dim,
            sdxl_context_dim=config.sdxl_context_dim, sdxl_pooled_dim=config.sdxl_pooled_dim, num_queries=config.num_latent_queries
    ...
    def generate(
        ...
        num_inference_steps: int = 25, guidance_scale: float = 4.5, seed: int = 42
    ) -> Image.Image:
        ...
        qwen_outputs = self.qwen.model(input_ids=encoded.input_ids, attention_mask=encoded.attention_mask, return_dict=True)
        ...
        # FIX 3: Dynamic Micro-conds to match the exact slider inputs
        ...
        # FIX 4: VAE strictly autocasted to float16 to prevent mantissa collapse
...
if __name__ == "__main__":
    ...
    img = pipe.generate(args.prompt, width=args.width, height=args.height, num_inference_steps=args.steps, guidance_scale=args.cfg, seed=args.seed)
