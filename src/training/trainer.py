...
class AdapterTrainer:
    ...
    def generate_validation_samples(self) -> None:
        ...
        for idx, prompt in enumerate(self.config.logging.validation_prompts):
            ...
            for i in range(20):
                ...
                with torch.autocast(device_type=self.device.type, dtype=self.dtype, enabled=self.use_amp):
                    eps_pred = self.unet(
                        dm_x.to(self.dtype), dm_t.to(self.dtype), 
                        encoder_hidden_states=adapter_ctx, added_cond_kwargs=added_cond_kwargs
                    ).sample
            ...
            validation_images[f"Validation/Sample_{idx}"] = wandb.Image(img_normalized.float().cpu(), caption=prompt)
    ...
    def train(self) -> None:
        ...
        progress_bar = tqdm(total=self.config.training.max_train_steps, desc="Training", initial=self.global_step)
        micro_step = 0
        ...
        while self.global_step < self.config.training.max_train_steps:
            for batch in self.train_dataloader:
                ...
                with torch.autocast(device_type=self.device.type, dtype=self.dtype, enabled=self.use_amp):
                    ...
                    loss = self.objective(
                        unet=self.unet, adapter_ctx=adapter_ctx, adapter_pooled=adapter_pooled,
                        x1=x1, micro_conds=micro_conds
                    )
